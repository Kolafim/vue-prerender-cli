...
//     if(tokens.length == 1){
//         console.log(tokens);
//         let _content = tokens[0].content.replace(/^\s+|\s+$/g,"");
//         if(_content) tokens[0].attrPush(['id', tokens[0].content]);
//     }
//     return self.renderInlineAsText(tokens, options, env);
// };
markdown.renderer.rules.link_open = function (tokens, idx, options, env, self) {
    // If you are sure other plugins can't add `target` - drop check below
    var aIndex = tokens[idx].attrIndex('target');
    if(!(tokens[idx].attrs[0] && tokens[idx].attrs[0][0] == "href" && (!tokens[idx].attrs[0][1] || tokens[idx].attrs[0][1].match(/^#/)))) {
        if (aIndex < 0) {
